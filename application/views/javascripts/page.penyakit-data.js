$(() => {
	...
	const editRow = (id, data) => {
		...
		$table.cell(row, 4).data(data.derajat_kepercayaan + ' %')
	}
	...
	$('#form').submit((ev) => {
		...
		let min = $('#min').val()
		let max = $('#max').val()
		let derajat_kepercayaan = $('#derajat_kepercayaan').val()

		if (id == 0) {

			// Insert

			window.apiClient.penyakitData.insert(nama, min, max, derajat_kepercayaan)
				.done((data) => {
			...
			window.apiClient.penyakitData.update(id, nama, min, max, derajat_kepercayaan)
				.done((data) => {
...
const Ubah = (id) => {
	window.apiClient.penyakitData.detail(id)
		.done((data) => {
			...
			$('#min').val(data.min)
			$('#max').val(data.max)
			$('#derajat_kepercayaan').val(data.derajat_kepercayaan)
